...
export default function Vegetables() {
  ...
  const filteredProducts = products.filter((prod) => {
    ...
    const matchesCategory = categoryName.toString().toLowerCase() === "vegetables";
  ...
  return (
    <div className="space-y-6 text-gray-100">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Manage your store vegetable inventory, variants, pricing, and stock.
          </p>
        </div>
      ...
      <div className="flex items-center justify-between rounded-md border border-gray-800 bg-gray-900/60 p-3 shadow-lg backdrop-blur-xl">
        <div className="relative w-full max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-md border border-gray-800 bg-gray-950/60 pl-9 pr-3 py-1.5 text-xs text-gray-100 placeholder-gray-500 outline-none focus:border-yellow-500 transition-colors"
          />
        </div>
      </div>
      ...
      {isError && (
        <div className="flex h-48 items-center justify-center rounded-md border border-rose-500/20 bg-rose-500/10 p-6 text-center text-xs font-medium text-rose-400">
          Failed to load vegetables. Please check your network or try again.
        </div>
      )}
      ...
      {!isLoading && !isError && filteredProducts.length === 0 && (
        <div className="flex h-48 flex-col items-center justify-center rounded-md border border-gray-800 bg-gray-900/40 p-6 text-center text-gray-400 space-y-2">
          ...
          <p className="text-xs font-medium">No vegetables found. Click "Add Vegetable" to create one.</p>
        </div>
      )}
