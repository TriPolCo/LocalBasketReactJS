...
export default function UpdateDeliveryPartnerModal({ isOpen, onClose, partner, onSuccess, isLoading = false }) {
  ...
  useEffect(() => {
    if (isOpen && partner) {
      const matchedVehicle = vehicleTypeOptions.find(
        (opt) => opt.value === partner.vehicle_type
  ...
  const handleSubmit = (e) => {
    ...
    if (onSuccess) {
      ...
      console.log("Updated Delivery Partner Payload:", payload);
      alert(`Delivery Partner "${form.first_name} ${form.last_name}" updated successfully!`);
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/80 p-4 animate-in fade-in duration-200 text-gray-100">
      <div className="relative w-full max-w-lg rounded-md border border-gray-800 bg-gray-900 p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        <h3 className="text-base font-bold text-white flex items-center gap-2">
          <Bike className="h-4 w-4 text-yellow-500" /> Update Delivery Partner
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-300 mb-1">
              Current Operating Location / Base Hub
            </label>
            <input
              type="text"
              disabled={isLoading}
          ...
          <div className="flex items-center pt-1">
            <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-300 select-none">
              <input
                ...
                className="rounded border-gray-800 bg-gray-950 text-yellow-500 focus:ring-0 h-3.5 w-3.5"
              />
